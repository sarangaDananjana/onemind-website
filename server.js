...
app.post('/api/projects/verify', (req, res) => {
    const { id, pin } = req.body;
    if (!id) return res.status(400).json({ error: 'Project ID is required' });

    try {
        const projectFile = path.join(PROJECTS_DIR, `${id}.json`);
        if (!fs.existsSync(projectFile)) return res.status(404).json({ error: 'Project not found' });
        
        const data = JSON.parse(fs.readFileSync(projectFile, 'utf-8'));
        
        if (data.status === 'undisclosed' && data.pin !== pin) {
            return res.status(401).json({ error: 'Invalid PIN' });
        }
        
        // PIN correct or project disclosed, generate a secure token for mobile viewing
        // In a real app we might store this in memory with an expiration, but a hash token works too
        const mobileToken = crypto.createHash('sha256').update(id + 'SECRET_SALT').digest('hex');
        
        // Remove pin from response for safety
        delete data.pin;
...
app.get('/api/projects/:id', (req, res) => {
    try {
        ...
        // Remove pin for safety
        delete data.pin;
        
        if (data.status === 'undisclosed') {
            const validToken = crypto.createHash('sha256').update(id + 'SECRET_SALT').digest('hex');
            if (token !== validToken) {
                // If invalid token, omit sensitive details
                delete data.price;
                delete data.reason;
                delete data.outcome;
                delete data.projectUrl;
            }
        }
